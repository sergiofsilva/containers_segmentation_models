...
DATASET_DIR = os.path.join(base_path,'dengue_novo_v8/')
...
print("Visualizing predictions for test dataset...")
os.makedirs(os.path.join(ROOT_PATH, 'predictions'), exist_ok=True)
for i in range(len(test_dataset)):
    print(f"Predicao para a imagem {test_dataset.images_fps[i]}")
    image, gt_mask, mask_rgb = test_dataset[i]
    image = np.expand_dims(image, axis=0)
    pr_mask = model.predict(image)

    file_with_ext = os.path.basename(test_dataset.images_fps[i])
    file_name, _ = os.path.splitext(file_with_ext)
        
    visualize(
        dir_to_save=os.path.join(ROOT_PATH, outputs_path),
        save_name=file_name,
        image=denormalize(image.squeeze()),
        gt_mask=bin_to_rgb(np.round(gt_mask.squeeze()),CLASSES,labels_dict),
        pr_mask=bin_to_rgb(np.round(pr_mask.squeeze()),CLASSES,labels_dict)
        )
